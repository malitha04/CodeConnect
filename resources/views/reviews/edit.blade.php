...
<div class="container mx-auto px-4 py-8">
    <div class="max-w-2xl mx-auto">
        <div class="mb-8">
            <h1 class="text-3xl font-bold text-text-primary">Edit Review</h1>
            <p class="text-text-secondary mt-2">Update your review for this project.</p>
        </div>
        ...
        <div class="bg-card-dark border border-border-custom rounded-xl p-6">
            ...
            <div class="mb-6">
                ...
                <div class="bg-secondary-dark rounded-lg p-4">
                    <h3 class="font-medium text-text-primary">{{ $review->project->title }}</h3>
                    <p class="text-text-secondary text-sm mt-1">{{ $review->project->description }}</p>
                    <div class="flex items-center mt-2 text-sm text-text-secondary">
                        <span class="mr-4"><i class="fas fa-user mr-1"></i>Developer: {{ $review->developer->name }}</span>
                        <span><i class="fas fa-dollar-sign mr-1"></i>Budget: ${{ number_format($review->project->budget) }}</span>
                    </div>
                </div>
            </div>
                ...
                <div class="space-y-6">
                        ...
                        <div class="flex items-center space-x-2">
                            @for($i = 1; $i <= 5; $i++)
                                <input type="radio" id="rating_{{ $i }}" name="rating" value="{{ $i }}" 
                                       class="hidden" {{ old('rating', $review->rating) == $i ? 'checked' : '' }} required>
                    ...
                    <div>
                        ...
                        <textarea id="comment" name="comment" rows="5" 
                                  ...
                                  class="w-full px-3 py-2 bg-secondary-dark border border-border-custom rounded-lg text-text-primary focus:outline-none focus:ring-2 focus:ring-accent-green focus:border-transparent">{{ old('comment', $review->comment) }}</textarea>
